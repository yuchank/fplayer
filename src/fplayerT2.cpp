...
int StreamComponentOpen(VideoState * is, int streamIndex)
{
	...
	else if (codecPar->codec_type == AVMEDIA_TYPE_VIDEO)
	{
		...
		is->hVideoThread = SDL_CreateThread(VideoThread, "VideoThread", is);
...
int main(int argc, char *argv[])
{
	...
	window = SDL_CreateWindow(filename,
		...
		SDL_WINDOWPOS_UNDEFINED,
		is->pFormatCtx->streams[videoStream]->codecpar->width,
		is->pFormatCtx->streams[videoStream]->codecpar->height,
		0);
	...
	is->hParseThread = SDL_CreateThread(DecodeThread, "DecodeThread", is);
